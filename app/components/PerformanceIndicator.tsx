...
export default function PerformanceIndicator({ data }: { data: Datashape }) {
  ...
  return (
    <div id={anchorId} className="bg-white dark:bg-black border border-neutral-300 dark:border-neutral-700 p-6 transition-colors duration-300 scroll-mt-24">
      <h4 className="text-lg font-light mb-2 text-black dark:text-white">{data.indicator}</h4>
      <p className="text-neutral-700 dark:text-neutral-300 whitespace-pre-wrap">{data.text}</p>
    </div>
  )
}
